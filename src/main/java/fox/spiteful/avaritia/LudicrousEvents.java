...
public class LudicrousEvents {
    ...
    public void onPlayerMine(PlayerInteractEvent event) {
        ...
        if (block.getBlockHardness(event.entityPlayer.worldObj, event.x, event.y, event.z) <= -1
            && event.entityPlayer.getHeldItem()
                .getItem() == LudicrousItems.infinity_pickaxe
            ...
            if (event.entityPlayer.getHeldItem()
                .getTagCompound() != null && event.entityPlayer.getHeldItem()
                    .getTagCompound()
                    .getBoolean("hammer")) {
                LudicrousItems.infinity_pickaxe
    ...
    public void onGetHurt(LivingHurtEvent event) {
        ...
        if (player.getHeldItem() != null && player.getHeldItem()
            .getItem() == LudicrousItems.infinity_sword && player.isUsingItem()) event.setCanceled(true);
    ...
    public void onLivingDrops(LivingDropsEvent event) {
        if (event.recentlyHit && event.entityLiving instanceof EntitySkeleton
            ...
            if (player.getHeldItem() != null && player.getHeldItem()
                .getItem() == LudicrousItems.skull_sword) {
    ...
    public void canHarvest(PlayerEvent.HarvestCheck event) {
        if (event.entityPlayer.getHeldItem() != null) {
            ...
            if (held.getItem() == LudicrousItems.infinity_shovel && event.block.getMaterial() == Material.rock) {
                if (held.getTagCompound() != null && held.getTagCompound()
                    .getBoolean("destroyer") && isGarbageBlock(event.block)) event.success = true;
            }
        }
    }
    ...
    public void onDeath(LivingDeathEvent event) {
        if (event.entityLiving instanceof EntityPlayer) {
            EntityPlayer player = (EntityPlayer) event.entityLiving;
            if (LudicrousItems.isInfinite(player) && !event.source.getDamageType()
                .equals("infinity")) {
    ...
    public void clusterClustererererer(EntityItemPickupEvent event) {
        if (event.entityPlayer != null && event.item.getEntityItem()
            .getItem() == LudicrousItems.matter_cluster) {
