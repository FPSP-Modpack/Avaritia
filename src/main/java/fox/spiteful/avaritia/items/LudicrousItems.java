...
public class LudicrousItems {
    ...
    public static void grind() {
        ...
        ultimate_stew = register(
            new ItemFood(20, 20.0f, false).setPotionEffect(Potion.regeneration.getId(), 300, 1, 1.0f)
                .setTextureName("avaritia:stew")
                .setUnlocalizedName("avaritia_stew")
                .setCreativeTab(Avaritia.tab),
            "Ultimate_Stew");
        cosmic_meatballs = register(
            new ItemFood(20, 20.0f, false).setPotionEffect(Potion.damageBoost.getId(), 300, 1, 1.0f)
                .setTextureName("avaritia:meatballs")
                .setUnlocalizedName("avaritia_meatballs")
                .setCreativeTab(Avaritia.tab),
            "Cosmic_Meatballs");
    ...
    public static boolean isInfinite(EntityPlayer player) {
        if (player.getEquipmentInSlot(1) == null || player.getEquipmentInSlot(2) == null
            ...
            || player.getEquipmentInSlot(4) == null) return false;
        if (player.getEquipmentInSlot(1)
            .getItem() == infinity_shoes
            && player.getEquipmentInSlot(2)
                .getItem() == infinity_pants
            && player.getEquipmentInSlot(3)
                .getItem() == infinity_armor
            && player.getEquipmentInSlot(4)
                .getItem() == infinity_helm)
            return true;
        else return false;
    }
